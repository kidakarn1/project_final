    ...
    $("#id_card_number").keyup(function(){
        ...
        $.each(data_patient.data,function(key,value){
          ...
          if(value.id_card_number==check_id_card_number){
            ...
            $('#Assessment_date').attr('disabled',true);//ปิด hn
            $('#score_ue').attr('disabled',true);//ปิด hn
            $('#score_le').attr('disabled',true);//ปิด hn
            $('#score_mo').attr('disabled',true);//ปิด hn
            $('#score_total').attr('disabled',true);//ปิด hn
            ...
            path = $.ajax({
              url:'query_main_assessment.php',
              type:"POST",
              datatype:"json",
              data:{
                id_card_number_auto:value.id_card_number,
              }
              })
              path.done(function(data){
                var length_groupby = JSON.parse(data);
                var data_main_assessment = JSON.parse(data);
                var data_score_mode = JSON.parse(data);
                console.log('data_main_assessment===>',data_main_assessment);
                var data_show_main_assessment ="";
                var check_limit_old = 0;
                var check_mode = 0;
                if (data_main_assessment.main_assessment!='0') {
                  $.each(length_groupby.length_groupby,function(key_main_assessment,value_main_assessment){
                    data_show_main_assessment+= "<div class='form-group row'>"
                    data_show_main_assessment+= "<label class='col-sm-2 col-form-label text-right'>วันที่ประเมิน:</label>"
                    data_show_main_assessment+="<div class='col-sm-4'>"
                    data_show_main_assessment+="  <input type='text' class='form-control' id='Assessment_date' value='"+value_main_assessment.assessment_date+"'disabled='disabled'>"
                    data_show_main_assessment+="</div>"
                    $.each(data_score_mode.main_assessment,function(key_mode,value_mode){
                      if (value_mode.Assessment_time==value_main_assessment.Assessment_time){
                        data_show_main_assessment+="<label class='col-sm-1 col-form-label text-right'>"+value_mode.ass_cat_id+":</label>"
                        data_show_main_assessment+="<div class='col-sm-1'>"
                        data_show_main_assessment+="  <input type='text'  class='form-control' id='score_le' value='"+parseInt(value_mode.avg_mode)+"' disabled='disabled'>"
                        data_show_main_assessment+="</div>"
                      }
                    });
                    ...
                    data_show_main_assessment+="</div>"
                    data_show_main_assessment+="<div class='col-sm-0'>"
                    data_show_main_assessment+="<button type='button' name='button'>ดู</button>"
                    data_show_main_assessment+="</div>"
                    data_show_main_assessment+="</div>";
                  });
              }

              $('#show_main_assessment').html(data_show_main_assessment);
              });
            return false;
          }
